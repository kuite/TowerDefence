...
public class TurretRed extends Towers {
    ...
    public TurretRed(Vector2 posE) {
        super(TextureManager.TOWERRED, new TextureRegion(TextureManager.LVL1RED), posE);
        setUpPos = posE;
    ...
    public void update() {
        ...
        if (System.currentTimeMillis() - lastFire >= 450) {
            ...
            if (Math.abs(lowestdx) <= 7 + TextureManager.RANGE1.getWidth() / 2 & Math.abs(lowestdy) <= 7 + TextureManager.RANGE1.getWidth() / 2) {
                if(lowestdy < 0 && lowestdx > 0){ //I
                    addMissile(new MissleRed(pos.cpy().add(-2 + 3 * TextureManager.LVL1RED.getWidth() / 4, 12 + TextureManager.LVL1RED.getHeight() / 2), new Vector2(-lowestdx / 6, -lowestdy / 5)));
