...
public class TurretBlue extends Towers {
    ...
    public TurretBlue(Vector2 posE) {
        super(TextureManager.TOWERBLUE, new TextureRegion(TextureManager.LVL1BLUE), posE);
        setUpPos = posE;
    ...
    public void update() {
        ...
        if (System.currentTimeMillis() - lastFire >= 300) {
            ...
            if (Math.abs(lowestdx) <= 7 + TextureManager.RANGE1.getWidth() / 2 & Math.abs(lowestdy) <= 7 + TextureManager.RANGE1.getWidth() / 2) {
                if(lowestdy < 0 && lowestdx > 0){ //I
                    addMissile(new MissleBlue(pos.cpy().add(-2 + 3 * TextureManager.LVL1RED.getWidth() / 4, 12 + TextureManager.LVL1RED.getHeight() / 2), new Vector2(-lowestdx / 6, -lowestdy / 5)));
